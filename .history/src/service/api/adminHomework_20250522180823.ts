...
export const batchDownloadHomework = async (userIds: string[], weeks: number): Promise<void> => {
    try {
        ...
        const downloadUrl = `/admin/batchDownloadHomework?userIds=${userIdsParam}&weeks=${weeks}`;
        ...
        try {
            ...
            // 设置默认文件名
            let fileName = `第${weeks}周作业汇总.zip`;
            if (contentDisposition) {
                const fileNameMatch = contentDisposition.match(/filename="(.+)"/);
                if (fileNameMatch && fileNameMatch[1]) {
                    fileName = fileNameMatch[1];
            ...
            // 确保设置正确的MIME类型，使用强制类型确保macOS兼容性
            const fileBlob = new Blob([blob], {
            ...
            // 显示提示信息
            ElMessage.info('文件下载中，如在macOS中无法打开，请尝试使用解压工具如The Unarchiver打开');
            ...
            console.error('批量下载文件失败，尝试使用window.open方式:', downloadError);

            // 提示用户使用备用下载方式
            ElMessage.warning('主下载方式失败，正在尝试备用方式，如下载后无法打开，请尝试使用专业解压工具');

            // 由于可能是浏览器限制或网络问题，尝试直接打开下载链接
            window.open(downloadUrl, '_blank');
        }
    } catch (error) {
