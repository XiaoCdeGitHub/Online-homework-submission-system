 ...
 * @returns Promise 返回是否已提交（1表示已提交，0表示未提交）
 */
export const checkHomeworkSubmitted = async (userId: string, homeworkId: string): Promise<HomeworkResponse<number>> => {
  try {
    if (!userId) {
      return {
        code: 400,
...
export const uploadHomework = async (formData: FormData): Promise<HomeworkResponse> => {
  try {
    ...
    const homeworkId = formData.get('homeworkId') as string; // 新增：获取作业ID
    ...
    reqData.append('homeworkId', homeworkId); // 新增：添加作业ID
