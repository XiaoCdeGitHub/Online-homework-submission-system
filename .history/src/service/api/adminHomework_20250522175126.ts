 ...
 * @returns Promise 下载结果
 */
export const batchDownloadHomework = async (userIds: string[], weeks: number, studentNames: string[] = []): Promise<void> => {
    try {
        ...
        const downloadPromises = userIds.map(async (userId, index) => {
            ...
            try {
                // 首先尝试获取该学生的作业提交信息
                const studentInfo = await getStudentHomeworkInfo(userId, weeks);
                if (!studentInfo || !studentInfo.fileUrl) {
                    failedCount++;
                    return `失败: ${studentName}的文件链接不存在`;
                }

                // 直接使用fileUrl下载文件
                const fileUrl = studentInfo.fileUrl;
                const response = await fetch(fileUrl);

                if (!response.ok) {
                    failedCount++;
                    return `失败: ${studentName}的文件下载失败(${response.status})`;
                }

                // 从URL获取原始文件名
                let fileName = fileUrl.split('/').pop() || `作业-${userId}`;
