...
const Submit = async () => {
  ...
  try {
    ...
    // 调用批量下载API
    await batchDownloadHomework(userIds, filterParams.weeks || currentWeeks.value);

    ElMessage.success(`正在下载${selectedStudents.length}个学生的作业汇总包`);
  } catch (error) {
