...
const Submit = async () => {
  ...
  try {
    ...
    for (const student of selectedStudents) {
      try {
        // 批量下载时不传递fileUrl，强制使用压缩包下载
        await downloadHomework(student.userId, filterParams.weeks || currentWeeks.value);
        downloadCount++;
      } catch (error) {
        console.error(`下载学生 ${student.user_name || student.name || '未知用户'} 的作业失败:`, error);
      }
    }
