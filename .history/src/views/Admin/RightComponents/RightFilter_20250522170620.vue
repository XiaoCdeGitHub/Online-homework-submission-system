...
import { getSelectCondition } from '@/service/api/adminHomework';
...
const handleSubmit = async () => {
  ...
  try {
    // 使用新的API获取筛选结果
    const response = await getSelectCondition(filterParams);

    if (response.code === 200) {
      ElMessage.success('查询成功');

      // 保存数据到本地存储
      localStorage.setItem("homeworkFilterResult", JSON.stringify(response));

      // 保存筛选参数到group store，便于其他组件使用
      const groupStore = useGroupStore();
      groupStore.setFilterParams(filterParams);
