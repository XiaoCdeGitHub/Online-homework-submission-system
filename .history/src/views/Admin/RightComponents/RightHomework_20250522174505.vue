...
const Submit = async () => {
  ...
  try {
    ...
    const studentNames = selectedStudents.map(student =>
      student.user_name || student.name || student.userName || `用户${student.userId}`
    );

    // 显示加载中提示
    ElMessage.info('正在准备批量下载，请稍候...');

    // 调用批量下载API，传递学生ID列表和名字列表
    await batchDownloadHomework(userIds, filterParams.weeks || currentWeeks.value, studentNames);
