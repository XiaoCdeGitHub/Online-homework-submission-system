...
import { downloadHomeworkFile, getRecentTask } from '@/service/api/homework';
...
const fetchHomeworks = async () => {
  ...
  try {
    ...
    console.log('正在请求作业数据:', { direction, weeks: currentWeek });
    ...
    if (res.data) {
      ...
      homeworkList.value = [{
        ...
        isSubmitted: homework.isActive === false // 假设isActive为false表示已提交
      }];

      console.log('获取本周作业成功:', homeworkList.value.length > 0 ? '有作业' : '无作业');
    } else {
      // 处理没有作业的情况
...
const getCurrentWeek = () => {
  // 这里只是一个示例，实际项目中可能需要从配置或API获取当前周数
  // 或从localStorage中获取
  try {
    const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
    return userInfo.currentWeek || 1; // 默认返回第1周
  } catch (error) {
