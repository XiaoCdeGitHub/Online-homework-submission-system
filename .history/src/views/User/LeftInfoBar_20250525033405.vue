...
// 使用新接口检查特定作业是否已提交
const checkHomeworkSubmitted = async (userId, homeworkId) => {
  try {
    // 使用新API检查特定作业的提交状态
    const res = await axios.get('/user/checkHomeworkSubmitted', {
      params: { userId, homeworkId }
    });
    console.log(`作业(ID: ${homeworkId})提交状态检查响应:`, res);
    return res;
  } catch (error) {
    console.error(`检查作业(ID: ${homeworkId})提交状态失败:`, error);
    return { code: -1, data: 0 }; // 发生错误时默认未提交
  }
};
  ...
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      try {
        // 如果有作业ID，使用新接口检查提交状态
        if (id) {
          const submitStatusRes = await checkHomeworkSubmitted(userId, id);
          if (submitStatusRes.code === 200) {
            isSubmitted = submitStatusRes.data === 1;
            console.log(`作业(ID: ${id})提交状态:`, isSubmitted ? '已提交' : '未提交');
          }
        } else {
          // 兼容旧代码：使用周数检查提交状态
      ...
      homeworkList.value = [{
        ...
        direction: direction,
        isSubmitted: isSubmitted // 使用新接口返回的提交状态
      }];
