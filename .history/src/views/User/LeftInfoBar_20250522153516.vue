...
const getUserInfoFromStorage = () => {
  ...
  console.warn('本地存储中没有有效用户信息或解析失败');
...
const fetchHomeworks = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 过滤有效的作业项目
      homeworkList.value = res.data.filter(item => item && item.id).map(item => ({
        ...item,
        title: item.title || `第${item.weeks}周作业`, // 确保有标题
        notice: item.notice || '暂无说明' // 确保有说明
      }));

      // 按周数排序，最新的周数排在前面
      homeworkList.value.sort((a, b) => b.weeks - a.weeks);

      console.log('获取作业列表成功:', homeworkList.value.length, '个作业');

      // 如果没有获取到作业，显示友好提示
      if (homeworkList.value.length === 0) {
        console.log('没有获取到可下载的作业');
        // 可以添加友好提示
        // ElMessage.info('暂无可下载作业');
      }
    } else if (res.code === -1) {
      // 特别处理没有作业的情况
      console.log('没有可下载的作业:', res.message);
      homeworkList.value = [];
    } else {
