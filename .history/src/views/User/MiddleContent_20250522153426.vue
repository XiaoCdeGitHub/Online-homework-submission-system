...
const getUserInfoFromStorage = () => {
  try {
    ...
    if (userInfoStr && userInfoStr !== 'undefined' && userInfoStr !== 'null') {
      ...
      // 转换数字组别为文本格式
      let groupText = '未分组'
      if (userInfo.group !== undefined && userInfo.group !== null && userInfo.group !== '') {
        // 更严格地检查组别是否为数字或可转为数字的字符串
        const groupNum = Number(userInfo.group)
        if (!isNaN(groupNum) && groupNum > 0) {
          groupText = `第${groupNum}组`
        } else if (typeof userInfo.group === 'string' && userInfo.group.trim() !== '') {
          groupText = userInfo.group // 如果是非空字符串，直接使用
        }
      }
...
const fetchCurrentTask = async () => {
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      // 处理没有最新作业的情况
      console.log('没有查询到最新作业，使用当前周数作为标题')
      progressTitle.value = `第${currentWeek.value}周 进度`
      // 可以添加友好提示
      // ElMessage.info('暂无本周作业信息')
    }
  } catch (error) {
