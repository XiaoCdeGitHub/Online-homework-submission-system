  ...
  try {
    ...
    if (res.code === 200 && res.data && res.data.homeworkList && Array.isArray(res.data.homeworkList) && res.data.homeworkList.length > 0) {
      ...
      const homeworks = homeworkList.map((homework) => {
        // 根据实际情况判断提交状态，默认为未提交
        const isSubmitted = homework.submitted === true ||
    ...
    if (res.code === 200 && res.data && res.data.homeworkUrlList && Array.isArray(res.data.homeworkUrlList) && res.data.homeworkUrlList.length > 0) {
      ...
      const homeworks = homeworkUrlList.map((url, index) => {
        ...
        return {
          ...
          direction: direction,
          isSubmitted: false, // 默认为未提交，稍后通过API查询更新
          isActive: true
        };
      });
...
const checkSubmissionStatus = async (userId, currentWeek) => {
  try {
    console.log(`开始检查用户${userId}第${currentWeek}周作业的提交状态`);

    // 参数检查
    if (!userId || !currentWeek) {
      console.warn('检查提交状态缺少必要参数', { userId, currentWeek });
      return false;
    }
    ...
    if (res.code === 200 && res.data) {
      ...
      console.log(`第${currentWeek}周作业提交状态详情:`, {
        hasSubmitted,
        submissionCount,
        submitTime,
        comments
      ...
      console.warn(`获取第${currentWeek}周作业提交状态失败:`, res.message || '未知错误');
      return false;
    }
  } catch (error) {
...
const getCurrentWeek = async () => {
  try {
    ...
    console.log('获取当前周数API响应:', response);
    ...
    if (response.code === 200 && response.data) {
      ...
      console.warn('API获取当前周数失败，尝试从localStorage获取');
    }
    ...
    if (weekFromStorage) {
      ...
      return Number(weekFromStorage); // 确保返回数字
    }
    ...
    try {
      const userInfo = JSON.parse(localStorage.getItem('userInfo') || '{}');
      const weekFromStorage = userInfo.currentWeek || userInfo.weeks || 1;
      console.log('从localStorage获取周数(错误恢复):', weekFromStorage);
      return Number(weekFromStorage); // 确保返回数字
    } catch (e) {
      console.error('从localStorage获取周数失败，使用默认值1:', e);
...
const fetchHomeworks = async () => {
  ...
  try {
    ...
    console.log(`当前周${currentWeekNum}的提交状态检查结果:`, hasSubmitted);
    ...
    if (res.code === 200 && res.data) {
      ...
      // 优化提交状态判断逻辑，只根据明确的提交标识判断
      // 移除isActive === false这个条件，因为它不能准确表示提交状态
