...
import { downloadHomeworkFile, getRecentTask, getAllTasks, getHistorySubmit, getSubmissionCount } from '@/service/api/homework';
...
const fetchAllHomeworks = async () => {
  try {
    ...
    try {
      const historyRes = await getHistorySubmit(userId);
      if (historyRes.code === 200 && Array.isArray(historyRes.data)) {
        submissionHistory = historyRes.data;
      }
    } catch (error) {
    ...
    if (res.code === 200 && res.data && res.data.length > 0) {
      ...
      allHomeworkList.value = res.data.map(homework => {
        ...
        const hasSubmitted = submissionHistory.some(submission =>
          submission.weeks === weekNumber ||
          submission.weeks === String(weekNumber)
        );
        ...
        return {
          ...
          isSubmitted: homework.isActive === false || hasSubmitted
        };
      });
...
const checkSubmissionStatus = async (userId, currentWeek) => {
  try {
    ...
    console.log('获取提交历史响应:', res);

    if (res.code === 200 && Array.isArray(res.data) && res.data.length > 0) {
      // 检查是否存在当前周数的提交记录
      const hasSubmitted = res.data.some(submission =>
        submission.weeks === currentWeek ||
        submission.weeks === String(currentWeek)
      );

      console.log(`第${currentWeek}周作业提交状态:`, hasSubmitted ? '已提交' : '未提交');
      return hasSubmitted;
    }
